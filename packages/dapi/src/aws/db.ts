...
const DynamoDBError = (msg: string, cause?: unknown) => new CustomError(msg, { name: 'DynamoDBError', cause });
...
const tryInit = async (silent = false) => {
  ...
  if (!silent) {
    throw DynamoDBError('Could not auto-initialize DynamoDB!', err);
  }
};
...
const writeItemForceHelper = async <T = any>(
  ...
  try {
    ...
    if (err.name === 'ConditionalCheckFailedException') {
      ...
      const msg = `PutCommandInput: ${JSON.stringify(cmdParams)}`
      if (numberOfAttempts === 1) {
        throw DynamoDBError(`${msg} [ERROR] An item with the same key(${(item as any)[key]}) already exists!`);
      } else {
        throw DynamoDBError(`${msg} [ERROR] Maximum attempts overflow!`);
      }
    }
    return null;
  }
...
const batchWriteItems = async (table: string, items: StringIndexable[]) => {
  ...
  try {
    ...
    const msg = `BatchWriteCommandInput: ${JSON.stringify(cmdParams)}`
    throw DynamoDBError(msg, err)
  }
...
export const updateItem = async (input: UpdateItemInput) => {
  ...
  try {
    ...
    const msg = `UpdateCommandInput: ${JSON.stringify(cmdParams)}`
    throw DynamoDBError(msg, err)
  }
...
export const readItem = async <T = any>(input: ReadItemInput) => {
  ...
  try {
    ...
    if (input.loud) {
      const msg = `GetCommandInput: ${JSON.stringify(cmdParams)}`
      throw DynamoDBError(msg, err)
    } else {
      return null;
    }
...
const batchReadItems = async <T = any>(
  ...
  try {
    ...
    const msg = `BatchGetCommandInput: ${JSON.stringify(cmdParams)}`
    throw DynamoDBError(msg, err)
  }
...
export const queryItems = async (input: QueryItemsInput) => {
  ...
  try {
    ...
    const msg = `QueryCommandInput: ${command.input}`
    throw DynamoDBError(msg, err)
  }
...
export const scanItems = async (input: ScanItemsInput) => {
  ...
  try {
    ...
    if (results.LastEvaluatedKey) {
      throw DynamoDBError('[scanItems] Partial results obtained! Consider pagination.');
    }
    ...
    const msg = `ScanCommandInput: ${JSON.stringify(cmdParams)}`
    throw DynamoDBError(msg, err)
  }
...
export const deleteItem = async (input: DeleteItemInput) => {
  ...
  try {
    ...
    const msg = `DeleteCommandInput: ${JSON.stringify(cmdParams)}`
    throw DynamoDBError(msg, err)
  }
