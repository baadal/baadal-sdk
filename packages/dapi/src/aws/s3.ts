...
const AWSS3Error = (msg: string, cause?: unknown) => new CustomError(msg, { name: 'AWSS3Error', cause });
...
const tryInit = async (silent = false) => {
  ...
  if (!silent) {
    throw AWSS3Error('Could not auto-initialize AWS S3!', err);
  }
};
...
export const putObject = async (bucket: string, s3path: string, contents: string) => {
  ...
  try {
    ...
    const msg = `BatchWriteCommandInput: ${JSON.stringify(baseParams)}`
    throw AWSS3Error(msg, err)
  }
...
export const uploadFile = async (bucket: string, file: string, s3path?: string) => {
  ...
  if (!contentType) {
    throw AWSS3Error(`Could not detect file type for: ${basename} [${filepath}]`)
  }
  ...
  try {
    ...
    const msg = `PutObjectCommandInput: ${JSON.stringify(baseParams)}`
    throw AWSS3Error(msg, err)
  }
...
export const getObject = async (bucket: string, s3path: string) => {
  ...
  try {
    ...
    const msg = `GetObjectCommandInput: ${JSON.stringify(cmdParams)}`
    throw AWSS3Error(msg, err)
  }
...
export const listObjects = async (bucket: string, prefix?: string) => {
  ...
  try {
    ...
    if (results.IsTruncated) {
      throw AWSS3Error('[listObjects] Partial results obtained! Consider pagination.');
    }
    ...
    const msg = `ListObjectsV2CommandInput: ${JSON.stringify(cmdParams)}`
    throw AWSS3Error(msg, err)
  }
...
export const getObjectHead = async (bucket: string, s3path: string) => {
  ...
  try {
    ...
    const msg = `HeadObjectCommandInput: ${JSON.stringify(cmdParams)}`
    throw AWSS3Error(msg, err)
  }
...
export const deleteObject = async (bucket: string, s3path: string) => {
  ...
  try {
    ...
    const msg = `DeleteObjectCommandInput: ${JSON.stringify(cmdParams)}`
    throw AWSS3Error(msg, err)
  }
...
const batchDeleteObjects = async (bucket: string, s3paths: string[]) => {
  ...
  try {
    ...
    const msg = `DeleteObjectsCommandInput: ${JSON.stringify(cmdParams)}`
    throw AWSS3Error(msg, err)
  }
